...
#if defined(NDEBUG)
# error "Token cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x722b81c95dea515b6159f24cd0a9bb8ec5fde39629a08e7c2dc21401de889d7f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Token: starting difficulty is 1 / 2^12
...
{
    // Token: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Token: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Token: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Token: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Token: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Token: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x722b81c95dea515b6159f24cd0a9bb8ec5fde39629a08e7c2dc21401de889d7f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Just another fucking day";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xc60c6425c2c64914de5817b7473779e6e32f3ff7c0c2650f706a7d63f81d67a2"));

// new merkel root code by daniel
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Token: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TokenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TokenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TokenMiner(CWallet *pwallet)
{
    printf("TokenMiner started\n");
    ...
    try { loop {
        ...
        printf("Running TokenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TokenMiner, pwallet));
}
